...
def upload(request, **kwargs):
    ...
    try:
        ...
        return simple_response(201, {"status": "ok", "file_name": uploader.upload(file, request)})
    except KeyError:
...
def delete(request):
    ...
    try:
        ...
        if type_ == "file":
            File.objects.get(hash=name).delete_file(request)
        elif type_ == "image":
            Image.objects.get(hash=name).delete_file(request)
        elif type_ == "skin":
            Skin.objects.get(hash=name).delete_file(request)
        elif type_ == "wallpaper":
            Wallpaper.objects.get(hash=name).delete_file(request)
