...
class UploadUrl(models.Model):
    ...
    def upload(self, file, request):
        ...
        if self.type == 'file':
            ...
            z = File(file=file, user=self.user, hash=hash_)
            z.save()
            request.in_history = True
            request.history = {"action": "create", "section": "media", "object": self.type,
                               "user": self.user, "params": {"name": z.name}}
            self.delete()
        elif self.type == 'image':
            if file.size > int(get_settings("images_max_size")):
                raise ZeroDivisionError
            z = Image(image=file, user=self.user, hash=hash_)
            z.save()
            request.in_history = True
            request.history = {"action": "create", "section": "media", "object": self.type,
                               "user": self.user, "params": {"name": z.name}}
            ...
            skin = z = Skin(image=file, user=self.user, hash=hash_)
            if self.to:
            ...
            if round(k, 2) != round(w / h, 2):
                raise InterruptedError
            z = Wallpaper(image=file, user=self.user, hash=hash_)
            z.save()
            request.in_history = True
            request.history = {"action": "create", "section": "media", "object": self.type,
                               "user": self.user, "params": {"name": z.name}}
...
class Image(models.Model):
    ...
    def delete_file(self, request):
        ...
        request.in_history = True
        request.history = {"action": "delete", "section": "media", "object": "image",
                           "user": request.user, "params": {"file": self.image.name, "creator": self.user.id}}
...
class Wallpaper(models.Model):
    ...
    def delete_file(self, request):
        storage, path = self.image.storage, self.image.path
        request.in_history = True
        request.history = {"action": "delete", "section": "media", "object": "wallpaper",
                           "user": request.user, "params": {"file": self.image.name, "creator": self.user.id}}
...
class File(models.Model):
    ...
    def delete_file(self, request):
        ...
        request.in_history = True
        request.history = {"action": "delete", "section": "media", "object": "file",
                           "user": request.user, "params": {"file": self.image.name, "creator": self.user.id}}
